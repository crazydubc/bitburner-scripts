...
const argsSchema = [
  ...
  ['reserved-ram', 40], // Keep this much home RAM free when scheduling hack/grow/weaken cycles on home.
...
export async function main(ns) {
  ...
  async function buildServerList(ns, verbose = false) {
    ...
    if (!useHacknetNodes)
      scanResult = scanResult.filter(hostName => !hostName.includes('hacknet-server-') && !hostName.includes('hacknet-node-'))
