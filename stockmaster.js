import {
  getConfiguration, getActiveSourceFiles, getBNMults,
  formatMoney, formatNumberShort, formatDuration, hasTixApi, getPlayerInfo, getReset, findPids, stockRun
} from './utils.js'
...
export async function main(ns) {
  ...
  const hasTixApiAccess = await hasTixApi(ns);
  if (runOptions.l || runOptions.liquidate) {
    ...
    log(ns, 'INFO: Killing any other stockmaster processes...', true, 'info');
    const pids = await findPids(ns, 'stockmaster.js');
    for (const pid of pids) {
      if (pid !== ns.pid) ns.kill(pid);
    }
    log(ns, 'INFO: Checking for and liquidating any stocks...', true, 'info');
  ...
  if ((await findPids(ns)).length > 1) return;
  ...
  allStockSymbols = await stockRun(ns, 'getSymbols')
  ...
  while (true) {
    ...
    if (pre4s) pre4s = !ns.stock.has4SDataTixApi();
...
async function getStockInfoDict(ns, stockFunction) {
  allStockSymbols ??= await stockRun(ns, 'getSymbols');//ns.stock.getSymbols();
  if (allStockSymbols == null) throw new Error(`No WSE API Access yet, this call to ns.stock.${stockFunction} is premature.`);
  return buildLib(ns, allStockSymbols, stockFunction);
  const entries = await Promise.all(
    allStockSymbols.map(async (sym) => [sym, await stockRun(ns, stockFunction, sym)])
  );
...
let launchSummaryTail = async ns => {
  ...
  if (ns.scriptRunning(summaryTailScript, ns.getHostname()))
    return;
...
let buyStockWrapper = async (ns, sym, numShares) => await stockRun(ns, 'buyStock', sym, numShares);
let buyShortWrapper = async (ns, sym, numShares) => await stockRun(ns, 'buyShort', sym, numShares);
let sellStockWrapper = async (ns, sym, numShares) => await stockRun(ns, 'sellStock', sym, numShares);
let sellShortWrapper = async (ns, sym, numShares) => await stockRun(ns, 'sellShort', sym, numShares);
...
async function doBuy(ns, stk, sharesToBuy) {
  ...
  let price = mock ? expectedPrice : long ? await buyStockWrapper(ns, stk.sym, sharesToBuy) : await buyShortWrapper(ns, stk.sym, sharesToBuy);
...
async function doSellAll(ns, stk) {
  ...
  let price = mock ? expectedPrice : long ? await sellStockWrapper(ns, stk.sym, sharesSold) : await sellShortWrapper(ns, stk.sym, sharesSold);
...
async function tryGet4SApi(ns, playerStats, budget) {
  ...
  if (playerStats.money < totalCost)
    await liquidate(ns);
  if (!(await stockRun(ns, 'purchase4SMarketData')))
  ...
  if ((await stockRun(ns, 'purchase4SMarketDataTixApi'))) {
...
async function tryGetStockMarketAccess(ns, budget) {
  if (await hasTixApi(ns)) return true; // Already have access
  ...
  if (!hasWSE) {
    if ((await stockRun(ns, 'purchaseWseAccount')))
  ...
  if ((await stockRun(ns, 'purchaseTixApi'))) {
