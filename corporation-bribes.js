import {
  corpRun, getOwnedAugs, getPlayerInfo, getReset, log, singRun
} from './utils.js'
import {
...
export const POST_ROUND_BRIBE_MAX_EXCESS_RATIO = 0.25
export const POST_ROUND_BRIBE_OPERATING_RESERVE_SECONDS = 10 * 60
const BRIBE_MANAGER_INTERVAL = 5000
...
export function getFactionBribeReserve(
  ...
  return Math.min(funds, Math.max(
...
/** Limit a bribe to the exact reputation gap and a bounded share of excess funds. */
export function calculateFactionBribeAmount({
  ...
  minimumReserve = POST_ROUND_BRIBE_MIN_RESERVE,
  reserveRatio = POST_ROUND_BRIBE_RESERVE_RATIO,
  maxExcessRatio = POST_ROUND_BRIBE_MAX_EXCESS_RATIO,
  operatingReserveSeconds = POST_ROUND_BRIBE_OPERATING_RESERVE_SECONDS,
}) {
  ...
  const excess = Math.max(0, funds - reserve)
  const required = Math.ceil((target - current) * costPerRep)
  const perPassCap = Math.max(0, excess * Math.max(0, Number(maxExcessRatio) || 0))
  const amount = Math.floor(Math.min(required, excess, perPassCap))
...
export async function main(ns) {
  ns.ramOverride(4)
  if (ns.ps('home').filter(process => process.filename === ns.getScriptName()).length > 1) return
  const resetInfo = await getReset(ns)

  while (true) {
    try {
      const offer = await corpRun(ns, 'getInvestmentOffer')
      if (Number(offer?.round) >= 5) await manageActiveFactionBribe(ns, resetInfo)
    } catch (error) {
      log(ns, `WARNING: corporation-bribes.js suppressed an unexpected error: ` +
        `${String(error?.stack ?? error)}`, false, 'warning')
    }
    await ns.sleep(BRIBE_MANAGER_INTERVAL)
  }
}
